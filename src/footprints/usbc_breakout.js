...
// Dimensions:
//   - Board: 20mm x 14mm
//   - NPTH: 2 mounting holes along the connector side
//   - Pins: 8 pins with 2.54mm spacing on opposite side
//
// Nets:
//   GND: Ground
//   VBUS: Power (5V)
...
module.exports = {
  ...
  body: p => {
    ...
    const npth_diameter = 2.5
    const pin_spacing = 2.54
    const pin_diameter = 1.0
    const pin_pad_size = 1.7

    // NPTH positions along the connector side (top)
    const npth_y = board_height / 2 - 2
    const npth_x_offset = 6
    ...
    return `
      (module USB-C_Breakout (layer ${p.side}.Cu) (tedit 5DD50112)
        ...
        (fp_text value "USB-C" (at 0 -9) (layer ${p.side}.Fab)
          (effects (font (size 1 1) (thickness 0.15)))
        )
        ...
        ${'' /* Mounting holes (NPTH) */}
        (pad "" np_thru_hole circle (at ${-npth_x_offset} ${npth_y} ${p.r}) (size ${npth_diameter} ${npth_diameter}) (drill ${npth_diameter}) (layers *.Cu *.Mask))
        (pad "" np_thru_hole circle (at ${npth_x_offset} ${npth_y} ${p.r}) (size ${npth_diameter} ${npth_diameter}) (drill ${npth_diameter}) (layers *.Cu *.Mask))
